...
- Tab naming is handled by the third-party `herdr-automatic-rename` plugin
  (`herdr plugin install qu8n/herdr-automatic-rename`): its zsh hook is
  sourced from `.zshrc`, `ui.prompt_new_tab_name = false` is set here, and
  `TITLE_BRANDS` in `.config/herdr-automatic-rename/config.sh` (stowed from
  this package) strips opencode's `OC |` title prefix. Agent tabs read
  `[N] <branch> · <task title>`; manual renames are respected, `reset`
  re-adopts a tab, and `doctor` explains a name. Names refresh on herdr's
  lifecycle events, so a session switch renames the tab on the next focus or
...
- The `last-reply` plugin reports sidebar metadata only: the `$last_reply`
  ...
  opencode session titles, generated by opencode's `small_model` (currently
  `openrouter/google/gemini-3.8-flash`); the plugin itself calls no model.
